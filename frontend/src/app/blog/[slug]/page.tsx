    ...
    <>
      ...
      <main className="py-[80px] px-6 md:px-8" style={{ background: 'var(--c-bg)' }}>
        <div className="max-w-[760px] mx-auto">
          ...
          {/* Linked services */}
          {blog.linkedServices?.length > 0 && (
            <div className="mt-8 pt-8 border-t flex flex-wrap items-center gap-3" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
              <span className="font-mono text-[0.5rem] tracking-[0.18em] uppercase" style={{ color: 'rgba(255,255,255,0.3)' }}>Related Services:</span>
              {blog.linkedServices.map((svc: any) => (
                <Link key={svc._id || svc} href={`/services/${svc.slug || ''}`}
                  className="font-mono text-[0.5rem] tracking-[0.12em] uppercase px-3 py-1.5 transition-all"
                  style={{ background: 'rgba(233,30,140,0.1)', color: '#e91e8c', borderRadius: '4px', border: '1px solid rgba(233,30,140,0.2)' }}>
                  {svc.name || svc}
                </Link>
              ))}
            </div>
