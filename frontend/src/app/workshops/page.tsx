    ...
    <>
      ...
      <main className="py-[80px] px-6 md:px-12" style={{ background: 'var(--c-bg)' }}>
        <div className="max-w-[1400px] mx-auto">
            ...
            <div className="flex flex-wrap gap-2 mb-10">
              ...
              {cats.map((cat: any) => (
                <button key={cat._id} onClick={() => setCat(cat._id)}
                  ...
                  style={{ borderRadius: '2px', background: catFilter === cat._id ? 'var(--c-gold)' : 'rgba(0,0,0,0.03)', color: catFilter === cat._id ? '#080808' : 'rgba(0,0,0,0.5)', border: `1px solid ${catFilter === cat._id ? 'var(--c-gold)' : 'rgba(0,0,0,0.08)'}` }}>
                  {cat.name}
                </button>
              ))}
            </div>
          ...
          {workshops.length === 0 ? (
            <div className="text-center py-24 font-mono text-[0.58rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(0,0,0,0.2)' }}>No workshops at the moment. Check back soon!</div>
          ) : (
