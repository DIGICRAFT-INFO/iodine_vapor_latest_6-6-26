          ...
          <div className="max-w-[1400px] mx-auto">
            <div ref={ref} className={`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${inView ? 'opacity-100' : 'opacity-0'} transition-opacity duration-700`}>
              <div>
                ...
                {aboutText && (
                  <div className="space-y-4 max-w-[480px]" style={{ color: 'rgba(245,240,234,0.5)', fontSize: '0.9rem', lineHeight: '1.9' }}>
                    <p>{aboutText}</p>
                  </div>
                )}
              </div>
            ...
            {clients.length > 0 && (
              <div className="mt-20">
                <p className="font-mono text-[0.58rem] tracking-[0.3em] uppercase mb-6" style={{ color: 'rgba(245,240,234,0.2)' }}>Brands We've Worked With</p>
                <div className="flex flex-wrap gap-3">
                  {clients.map((c: string) => (
                    <span key={c} className="font-mono text-[0.6rem] tracking-[0.15em] uppercase px-4 py-2.5 border" style={{ borderColor: 'rgba(255,255,255,0.07)', color: 'rgba(245,240,234,0.4)', borderRadius: '2px' }}>{c}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {testimonials.map((t: any) => (
                  <motion.div key={t._id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                    className="p-6 border" style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.06)', borderRadius: '2px' }}>
                    <div className="flex gap-1 mb-4">
                      {Array.from({ length: t.rating }).map((_, i) => <span key={i} style={{ color: 'var(--c-gold)', fontSize: '0.75rem' }}>★</span>)}
                    </div>
                    <p className="text-[0.85rem] leading-[1.8] mb-5" style={{ color: 'rgba(245,240,234,0.55)', fontStyle: 'italic' }}>"{t.content}"</p>
                    <div>
