    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
              <h2 className="font-display text-[1.4rem] tracking-[0.06em] text-cream-DEFAULT">{title}</h2>
              <button onClick={onClose} className="font-mono text-[0.7rem] transition-colors" style={{ color: 'rgba(245,240,234,0.3)' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = '#f5f0ea'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = 'rgba(245,240,234,0.3)'}>✕</button>
            </div>
            {/* Body */}
...
export function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <div>
      <label className="block font-mono text-[0.52rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(245,240,234,0.4)' }}>
    ...
    <div className="border" style={{ borderColor: 'rgba(255,255,255,0.08)', borderRadius: '2px' }}>
      <button type="button" onClick={() => setOpen(!open)}
        ...
        <div className="flex items-center gap-3">
          <span className="font-mono text-[0.58rem] tracking-[0.15em] uppercase" style={{ color: 'rgba(245,240,234,0.45)' }}>{label}</span>
          {value?.text && <span className="text-[0.7rem] truncate max-w-[200px]" style={{ color: 'rgba(245,240,234,0.3)' }}>{value.text}</span>}
        </div>
        <span className="font-mono text-[0.6rem]" style={{ color: 'rgba(245,240,234,0.3)', transform: open ? 'rotate(180deg)' : 'none', display: 'inline-block', transition: 'transform 0.2s' }}>▾</span>
      </button>

      {open && (
        <div className="px-4 pb-4 pt-3 space-y-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* Color */}
            <div>
              <p className="font-mono text-[0.48rem] tracking-[0.18em] uppercase mb-1.5" style={{ color: 'rgba(245,240,234,0.3)' }}>Color</p>
            ...
            {/* Font Size */}
            <div>
              <p className="font-mono text-[0.48rem] tracking-[0.18em] uppercase mb-1.5" style={{ color: 'rgba(245,240,234,0.3)' }}>Font Size</p>
            ...
            {/* Font Weight */}
            <div>
              <p className="font-mono text-[0.48rem] tracking-[0.18em] uppercase mb-1.5" style={{ color: 'rgba(245,240,234,0.3)' }}>Weight</p>
            ...
            {/* Font Family */}
            <div>
              <p className="font-mono text-[0.48rem] tracking-[0.18em] uppercase mb-1.5" style={{ color: 'rgba(245,240,234,0.3)' }}>Font Family</p>
          ...
          {/* Text Align */}
          <div>
            <p className="font-mono text-[0.48rem] tracking-[0.18em] uppercase mb-1.5" style={{ color: 'rgba(245,240,234,0.3)' }}>Text Align</p>
            <div className="flex gap-2">
              {ALIGNS.map(a => (
                <button key={a} type="button" onClick={() => onChange({ ...value, textAlign: a })}
                  ...
                  style={{ borderRadius: '2px', background: value?.textAlign === a ? 'var(--c-gold)' : 'rgba(255,255,255,0.05)', color: value?.textAlign === a ? '#080808' : 'rgba(245,240,234,0.4)' }}>
                  {a[0].toUpperCase()}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-4">
            {[{ key: 'italic', label: 'Italic' }, { key: 'uppercase', label: 'Uppercase' }].map(t => (
              <label key={t.key} className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="font-mono text-[0.52rem] tracking-[0.12em] uppercase" style={{ color: 'rgba(245,240,234,0.45)' }}>{t.label}</span>
              </label>
            ))}
          </div>
          ...
          {/* Preview */}
          {value?.text && (
            <div className="p-3 border" style={{ borderColor: 'rgba(255,255,255,0.05)', borderRadius: '2px', background: 'rgba(8,8,8,0.5)' }}>
              <p className="font-mono text-[0.45rem] tracking-[0.2em] uppercase mb-2" style={{ color: 'rgba(245,240,234,0.2)' }}>Preview</p>
              <span style={{
                color:       value.color,
...
export function Toggle({ label, checked, onChange, color = 'var(--c-gold)' }: { label: string; checked: boolean; onChange: (v: boolean) => void; color?: string }) {
  return (
    <label className="flex items-center gap-3 cursor-pointer">
      ...
      <span className="font-mono text-[0.55rem] tracking-[0.15em] uppercase" style={{ color: 'rgba(245,240,234,0.5)' }}>{label}</span>
    </label>
  );
}
...
export function PositionPicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div>
      <p className="font-mono text-[0.48rem] tracking-[0.18em] uppercase mb-2" style={{ color: 'rgba(245,240,234,0.3)' }}>Content Position</p>
      <div className="inline-grid gap-1" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
        {POSITIONS.flat().map(pos => (
          <button key={pos} type="button" onClick={() => onChange(pos)}
            ...
            style={{
              borderRadius: '2px',
              background:   value === pos ? 'var(--c-gold)' : 'rgba(255,255,255,0.04)',
              border:       `1px solid ${value === pos ? 'var(--c-gold)' : 'rgba(255,255,255,0.08)'}`,
              fontSize: '0.45rem',
              color: value === pos ? '#080808' : 'rgba(245,240,234,0.3)',
            }}
            title={pos}
          >
      ...
      <p className="font-mono text-[0.46rem] tracking-[0.12em] mt-1.5" style={{ color: 'rgba(245,240,234,0.25)' }}>{value}</p>
    </div>
  );
}
...
export function MediaPicker({ onSelect, onClose, type = 'all' }: { onSelect: (url: string, id?: string) => void; onClose: () => void; type?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(16px)' }}>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="font-mono text-[0.65rem]" style={{ color: 'rgba(245,240,234,0.3)' }}>✕</button>
          </div>
        </div>
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-w-0 border-r" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
            {/* Toolbar */}
            <div className="flex items-center gap-3 px-4 py-3 border-b flex-shrink-0" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
            ...
            <div {...getRootProps()} className="mx-4 mt-3 flex-shrink-0 px-4 py-3 border-2 border-dashed text-center cursor-pointer transition-all"
              style={{ borderColor: isDragActive ? 'var(--c-gold)' : 'rgba(255,255,255,0.08)', background: isDragActive ? 'rgba(201,169,110,0.06)' : 'transparent', borderRadius: '2px' }}>
              <input {...getInputProps()} />
              <p className="font-mono text-[0.56rem] tracking-[0.18em] uppercase" style={{ color: isDragActive ? 'var(--c-gold)' : 'rgba(245,240,234,0.3)' }}>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-32" style={{ color: 'rgba(245,240,234,0.2)' }}>
                ...
                <div className="flex justify-center gap-2 mt-4">
                  {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
                    <button key={p} onClick={() => setPage(p)}
                      ...
                      style={{ borderRadius: '2px', background: page === p ? 'var(--c-gold)' : 'rgba(255,255,255,0.05)', color: page === p ? '#080808' : 'rgba(245,240,234,0.4)' }}>{p}</button>
                  ))}
                </div>
              )}
            </div>
          ...
          {selItem && (
            <div className="w-52 flex-shrink-0 p-4 flex flex-col gap-3 overflow-y-auto">
              <p className="font-mono text-[0.5rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(245,240,234,0.3)' }}>Selected</p>
              <div className="aspect-square overflow-hidden" style={{ background: '#1a1a1a', borderRadius: '2px' }}>
                {selItem.type === 'image' ? <img src={imgUrl(selItem.url)} alt="" className="w-full h-full object-contain" /> : <div className="w-full h-full flex items-center justify-center font-mono text-[0.6rem]" style={{ color: 'rgba(245,240,234,0.3)' }}>{selItem.type}</div>}
              </div>
              <div className="space-y-1.5 text-[0.7rem]" style={{ color: 'rgba(245,240,234,0.4)' }}>
                <p className="truncate"><span style={{ color: 'rgba(245,240,234,0.2)' }}>Name: </span>{selItem.originalName}</p>
                <p><span style={{ color: 'rgba(245,240,234,0.2)' }}>Type: </span>{selItem.type}</p>
                <p><span style={{ color: 'rgba(245,240,234,0.2)' }}>Size: </span>{(selItem.size / 1024).toFixed(1)} KB</p>
              </div>
              <button onClick={() => onSelect(imgUrl(selItem.url), selItem._id)}
  ...
  return (
    <div className="fixed inset-0 z-[700] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)' }}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }}
        ...
        <p className="text-[0.78rem] mb-6" style={{ color: 'rgba(245,240,234,0.4)' }}>This action cannot be undone.</p>
